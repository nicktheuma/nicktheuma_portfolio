...
function isHeicObjectKey(objectKey) {
  const extension = path.posix.extname(objectKey).toLowerCase()
  return extension === '.heic' || extension === '.heif'
}

function convertedObjectKey(originalKey) {
  const { dir, name } = path.posix.parse(originalKey)
  const generatedDir = dir ? `${dir}/.generated` : '.generated'
  return `${generatedDir}/${name}.jpg`
}

async function ensureConvertedR2Image(s3, bucket, originalKey) {
  if (!isHeicObjectKey(originalKey)) {
    return originalKey
  }

  const convertedKey = convertedObjectKey(originalKey)

  // Check if converted version already exists in R2
  try {
    await s3.send(
    ...
    // Converted file exists, use it
    return convertedKey
  } catch {
    // Converted file doesn't exist, need to create it
  }

  // Download original HEIC from R2
  ...
  const heicBuffer = await originalResponse.Body?.transformToByteArray?.()
  if (!heicBuffer) {
    throw new Error(`Failed to download ${originalKey}`)
  }

  // Try to convert using Sharp first (faster), fallback to heic-convert
  let jpegBuffer
  try {
    jpegBuffer = await sharp(heicBuffer)
      .rotate()
      .jpeg({ quality: 92, mozjpeg: true })
      .toBuffer()
  } catch {
    // Sharp failed, use heic-convert
  ...
  // Upload converted JPEG back to R2
  await s3.send(
    ...
    sortedSlugs.map(async (slug) => {
      ...
      const images = await Promise.all(
        imageKeys.map(async (objectKey, index) => {
          // Convert HEIC/HEIF files to JPEG in R2 if needed
          const displayObjectKey = await ensureConvertedR2Image(s3, bucket, objectKey)
          ...
          // Generate thumbnail from R2 object (use converted version if HEIC)
          let thumbnail = ''
          try {
          ...
          return {
            ...
            src: mediaPathFromObjectKey(displayObjectKey),
